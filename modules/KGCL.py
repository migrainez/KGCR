...
from .HAggregator import AttnHGCN
...
class KGCL(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat, hp_dict=None):
        ...
        self.ablation = args_config.ablation
    ...
    def forward(self, batch=None, ab=0):
        user = batch['users']
    ...
    def create_mae_loss(self, node_pair_emb, masked_edge_type=None):
        head_embs, tail_embs = node_pair_emb[:, 0, :], node_pair_emb[:, 1, :]
        masked_intent_emb = self.intent_emb[masked_edge_type - 1]
        masked_relation_emb = self.gcn.relation_emb[masked_edge_type - 1]
        masked_edge_emb = masked_relation_emb * masked_intent_emb
    ...
    def print_shapes(self):
        ...
        self.logger.info("ra_coef: {}".format(self.ra_coef))
